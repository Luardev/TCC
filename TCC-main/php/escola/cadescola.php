...
<body class='bg-secondary'>
    ...
    <h1 class='text-center mt-4 mb-5'> Campeonatos da Escola: <?php echo htmlspecialchars($_SESSION['login']) ?></h1>
      ...
      foreach ($dados as $row){
        echo'<tr> 
            <th scope="col"> '.htmlspecialchars($row['ID_CAMP']).' </th>
            <td> '.htmlspecialchars($row['NOME']).'</td>
            <td> '.htmlspecialchars($row['QUANTIDADE']).'</td>
            <td> '.$row['Data_format'].'</td>
      ...
      echo "
        ...
        <div class="card" style="width: 22rem; margin:auto;">
            <div class="card-body">
              <h5 class="card-title" style="">Nenhum campeonato cadastrado</h5>
    ...
    <div class='modal-dialog modal-dialog-centered'>
      <div class='modal-content'>
        ...
        <div class='modal-footer'>
          ...
          <a id='cad_new_camp' class='btn btn-primary' data-bs-toggle='modal' data-bs-target='#Cadatrar_jogadores'>Proximo passo</a>
          <!--</form>-->
        </div>
      </div>
    </div>
  </div>
  <!-- Modal Cadastrar PT-2 -->
  <div class='modal' id='Cadatrar_jogadores' tabindex='-1' aria-labelledby='exampleModalLabel' aria-hidden='true'>
    <div class='modal-dialog modal-dialog-centered'>
      <div class='modal-content'>
        <div class='modal-header'>
          <h1 class='modal-title fs-5' id='exampleModalLabel'>Cadastrar Jogadores</h1>
          <button type='button' class='btn-close' data-bs-dismiss='modal' aria-label='Close'></button>
        </div>
        <div class='modal-body' id='ModCad2'>
            <div class='mb-3'>
              <label for='Nome_jogador' class='col-form-label'>Nome do jogador</label>
              <input type='text' class='form-control' name='Nome_jogador' placeholder="Digite o nome do jogador..." required>
            </div>
        </div>
        <div class='modal-footer'>
          <button type='button' class='btn btn-secondary' data-bs-dismiss='modal'>Cancelar</button>
          <button id='cad_new_camp' class='btn btn-primary' onclick='addjogador()'>Adicionar jogador</button>
          <button id='cad_new_camp' class='btn btn-primary'>Cadastrar</button>
          </form>
        </div>
      </div>
    </div>
